...
class Ui_VistaStipendio(QWidget):
    def __init__(self,keys, parent=None):
        super(Ui_VistaStipendio, self).__init__(parent)
        ...
        VisualizzaWindow.setWindowTitle(self._translate("VisualizzaWindow", "VisualizzaStipendio"))
        ...
        'giorni_ferie':' ferie',
        'importo_stipendio':' imp. stipendio',
        ...
        for a in self.viewList:
            for y in a:
                if(y != "dipendente" and y != "transazione_stipendio"):
                    ...
                    if(len(self.viewList[0]['transazione_stipendio'])>0 and y != "dipendente"):
                        self.AddSectionLabel('Transazione_stipendio')
                        self.StipendioGridLayout,self.scrollAreaWidgetContents = self.AddScrollArea(0)
                        self.count2 = self.count
                        self.count3 = self.count
                        for x in self.viewList[0]['transazione_stipendio']:
