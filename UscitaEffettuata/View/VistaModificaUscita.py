...
import copy
import UscitaEffettuata.View.UscitaEffettuataV as Vista

class Ui_ModificaUscita(QWidget):
    def __init__(self,key, parent=None):
        super(Ui_ModificaUscita, self).__init__(parent)
        ...
        ModificaWindow.setWindowTitle(self._translate("ModificaWindow", "ModificaUscita"))
        self.verticalLayout = QtWidgets.QVBoxLayout(ModificaWindow)
        self.verticalLayout.setObjectName("verticalLayout")

        #Aggiunge il "titolo" della finestra
        self.AddLabelTitolo("Modifica Uscita")
        ...
        self.body = {
            'id_transazione': self.chiamata['id_transazione'],
        ...
        self.exclude = ['id_transazione','transazione_stipendio','transazione_altra_spesa','transazione_retribuzione']
        ...
        for a in self.viewList:
            ...
            if(a == 'data_esecuzione'):
                self.listaInput[a] = self.AddCalendar(name='data_esecuzione',value=self.chiamata[a])
            else:
    ...
    def Modify(self):
        ...
        if('message' in self.risultato):
            ...
            self.OpenLista = Vista.Ui_ListaUscita()
            self.OpenLista.show()
            self.close()
