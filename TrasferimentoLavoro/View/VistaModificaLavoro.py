...
class Ui_ModificaTrasf(QWidget):
    def __init__(self,key, parent=None):
        super(Ui_ModificaTrasf, self).__init__(parent)
        self._translate = QtCore.QCoreApplication.translate
        self.key = key
        self.controller = TrasferimentoLavoroC()
        print(self.key)
        self.chiamata = self.controller.GetKey(str(self.key))
        print(self.chiamata)
        ModificaWindow = self
        ModificaWindow.setObjectName("ModificaWindow")
        ModificaWindow.resize(800, 600)
        ModificaWindow.setWindowTitle(self._translate("ModificaWindow", "Modifica Lavoro"))
        self.verticalLayout = QtWidgets.QVBoxLayout(ModificaWindow)
        self.verticalLayout.setObjectName("verticalLayout")

        #Aggiunge il "titolo" della finestra
        self.AddLabelTitolo("Modifica Lavoro")
        ...
        for a in self.viewList:
            ...
            if(a == 'dipendente'):
                print(self.chiamata[a])
                self.listaInput[a] = self.AddDropDown(contentList = list(self.DipendentiDict.keys()),name='dipendente',value=self.DipDict[self.chiamata[a]])
            elif(a == 'commessa'):
                self.listaInput[a] = self.AddField(self.chiamata[a]['codice_merce'])
            elif(a == 'data_conclusione'):
                self.listaInput[a] = self.AddCalendar(name='data_conclusione',value=self.chiamata[a])
            else:
    ...
    def Modify(self):
        ...
        for a in self.listaInput:
            ...
            elif(self.listaInput[a].objectName() == 'data_conclusione'):
        ...
        if('message' in self.risultato):
            ...
            self.OpenLista = ListView.Ui_ListaTrasf()
            self.OpenLista.show()
            self.close()
