...
class Ui_InsertDDT(QWidget):
    def __init__(self, parent=None):
        super(Ui_InsertDDT, self).__init__(parent)
        self._translate = QtCore.QCoreApplication.translate
        
        self.controller = DdtC()
        InsertWindow = self
        InsertWindow.setWindowTitle(self._translate("Inserimento DDT", "Inserimento DDT"))
        InsertWindow.setObjectName("Inserimento DDT")
        ...
        for a in self.traduzione:
            ...
            if(a == 'tipo_ddt'):
                self.listaInput[a] = self.AddDropDown()
            elif(a == 'data'):
                print('abbiamo data')
                self.listaInput[a] = self.AddCalendar()
            else:
    ...
    def AddDropDown(self):
        ...
        self.verticalLayout_2.addLayout(self.horizontalLayout)
        self._translate = QtCore.QCoreApplication.translate
        DropDownSelect.addItem("contabile")
        DropDownSelect.addItem("Non contabile")
        DropDownSelect.setCurrentIndex(0)
    ...
    def Insert(self):
        ...
        for a in self.listaInput:
            if(a == 'tipo_ddt'):
                tipo_ddt = self.listaInput['tipo_ddt'].currentIndex()
                continue
            elif(a=='data'):
        ...
        if(tipo_ddt == 0):
            self.risultato = self.controller.ContabileInsert(self.body)
        else:
            self.risultato = self.controller.NonContabileInsert(self.body)
