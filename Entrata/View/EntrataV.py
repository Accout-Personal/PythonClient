...
class Ui_ListaEntrata(QWidget):
    def __init__(self, parent=None):
        super(Ui_ListaEntrata, self).__init__(parent)
        self._translate = QtCore.QCoreApplication.translate
        ListaEntrata = self
        self.controller = EntrataC()
        self.key = 'numero_fattura'
        self.chiamata = self.controller.GetAll()
        ListaEntrata.setObjectName("ListaEntrata")
        ListaEntrata.resize(800, 600)
        ListaEntrata.setWindowTitle(self._translate("ListaEntrata", "ListaEntrata"))
        self.verticalLayout = QtWidgets.QVBoxLayout(ListaEntrata)
        ...
        'data_versamento':'data del versamento'
        }

        self.listAttr = ['numero_fattura','data','importo_entrata','data_versamento']
        ...
        QtCore.QMetaObject.connectSlotsByName(ListaEntrata)

    #chiamata alla funzione per la cancellazione di un elemento
    def deleteConfirm(self,entrata):

        msg = QMessageBox()
        msg.setWindowTitle('Conferma')
        msg.setText('sei sicuro di voler cancellare l\' entrata '+str(entrata['numero_fattura']) + '?')
        ...
        if(msg.clickedButton() == okButton):
            ...
            postbody = {self.key:entrata[self.key]}
            res = self.controller.Delete(postbody)
            print(res)
            self.RefreshLista = Ui_ListaEntrata()
    ...
    def AddTableHeader(self,text,pos,width):
        ...
        self.header1.setText(self._translate("ListaEntrata",text))
    ...
    def AddTableContent(self,x,y,content,width):
        ...
        self.label_3.setText(self._translate("ListaEntrata","   "+ str(content)))
    ...
    def AddOperationButton(self,x,y,buttonDict,function):
        ...
        OperationButton.setText(self._translate("ListaEntrata", buttonDict["name"]))        
