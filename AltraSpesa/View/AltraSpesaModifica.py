...
class Ui_ModificaAltraSpesa(QWidget):
    def __init__(self,key, parent=None):
        super(Ui_ModificaAltraSpesa, self).__init__(parent)
        ...
        self.exclude = ['codice','transazione_altra_spesa']
        ...
        for a in self.viewList:
            ...
            if(a == 'data'):
                self.listaInput[a] = self.AddCalendar('data',self.chiamata[a])
            elif(a == 'scadenza'):
                self.listaInput[a] = self.AddCalendar('scadenza',self.chiamata[a])
            else:
